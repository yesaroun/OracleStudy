...
( NO    NUMBER
, NAME  VARCHAR2(10)            -- VARCHAR2 문자라는 뜻임
, ADDR  VARCHAR2(20)
);
...
-- 부서번호 부서명  부서위치
-- ex)
-- 10       인사부 서울      -> 데이터 입력 가능하다.
-- 20               대전     -> 데이터 입력 가능
-- 30       개발부           -> 데이터 입력 가능
--          총무부  대구     -> 데이터 입력 불가~!!!(not null 이여서)
