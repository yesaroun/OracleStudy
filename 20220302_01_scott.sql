...
--○ 추가 데이터 입력 → 2001년부터 시작된주문이 현재(2022)까지 계속 발생~!!
...
98764	고래밥	10	2022-03-02 09:30:00
98765	빼빼로	20	2022-03-02 09:30:38
98766	맛동산	30	2022-03-02 09:31:09
98767	홈런볼	40	2022-03-02 09:31:32
98768	오감자	50	2022-03-02 09:31:56
98769	웨하스	30	2022-03-02 09:32:23
98770	고래밥	20	2022-03-02 09:33:07
98771	맛동산	20	2022-03-02 09:33:39
98772	웨하스	20	2022-03-02 09:33:56
98773	빼빼로	90	2022-03-02 09:34:18
98774	에이스	20	2022-03-02 09:34:36
...
-- 어플리케이션과 연동으로 인해 주문 내역을 다른 테이블에
...
-- TBL_JUMUN 테이블의 데이터들 중
-- 백업을 마친 데이터들 삭제 → 즉, 금일 발생할 주문 내역이 아닌 데이터들 제거
SELECT *
  FROM TBL_JUMUN
 WHERE TO_CHAR(JUDAY, 'YYYY-MM-DD') != TO_CHAR(SYSDATE, 'YYYY-MM-DD');
...
--   중복을 제거하고 1개 행만 조회한 결과를 반환하게 된다.

--○ 지금까지 주문받은 데이터를 통해
--   제품별 총 주문량을 조회할 수 있는 쿼리문을 구성한다.

-- 내 풀이
SELECT T.JECODE "제품", SUM(T.JUSU)
FROM
(
    SELECT *
    FROM TBL_JUMUNBACKUP
...
-- 풀이
SELECT T.JECODE "제품코드", SUM(T.JUSU) "주문 수량"
FROM
(
    SELECT JECODE, JUSU
    FROM TBL_JUMUNBACKUP
...
98774	에이스	20	2022-03-02 09:34:36
98775	꼬북칩	30	2022-03-02 09:34:57
98776	모옹쉘	30	2022-03-02 11:28:19
*/
...
--==>>
/*
98778	빼빼로	40	2022-03-02 14:08:44
98779	맛동산	20	2022-03-02 14:09:09
98780	모옹쉘	20	2022-03-02 14:13:50
98781	빼빼로	30	2022-03-02 14:13:51
98764	고래밥	10	2022-03-02 09:30:00
98765	빼빼로	20	2022-03-02 09:30:38
98766	맛동산	30	2022-03-02 09:31:09
98767	홈런볼	40	2022-03-02 09:31:32
...
98775	꼬북칩	30	2022-03-02 09:34:57
98776	모옹쉘	30	2022-03-02 11:28:19
98777	모옹쉘	10	2022-03-02 14:07:56
*/
...
--모옹쉘	30 / 빼빼로	20 / 빼빼로	30 여기에 / 모용쉘 20이 포함되서 나온거임
...
SELECT EMPNO, ENAME, JOB, SAL
 ...
 WHERE SAL = (SELECT MAX(SAL)
                FROM TBL_EMP)
   AND DEPTNO = 20;
--==>> 조회 결과 없음     --(위에서 구한 최대 급여를 받는 사람이 20번이 아니기때문임)
...
SELECT EMPNO, ENAME, JOB
FROM EMP
WHERE EMPNO IN (7902, 7698, 7698, 7698, 7839, 7839, 7566, NULL,  7839, ...); --(이렇게 찾는데 중복된걸 처리해주는 함수가 DISTINCT)
...
SELECT EMPNO, ENAME, JOB
FROM EMP
WHERE EMPNO IN (7902, 7698, 7839, 7566, NULL,  ...); --(이렇게 중복된거 한번만 찾아줌)
...
SELECT DEPTNO, DNAME, ENAME, SAL
  FROM EMP NATURAL JOIN DEPT;           -- (자연스럽게 조인해달라니까 알아서 해줌/ 이런게 있다만 알아보고 적극적으로 쓰지는 말자)
--==>>
/*
10	ACCOUNTING	CLARK	2450
10	ACCOUNTING	KING	5000
...
--==>> Session이(가) 변경되었습니다.
-- (나중에 개발자되서 데이트 형태 바꾸고 싶다고 세션 변경하면 안된다 TO_CHAR 로 바꿔서 꺼내야함)
