...
--■■■ 정규화(Normalization) ■■■-- (중요 충분히 복습해야함!)
...
가정) 서울 여의도 LG라는 회사에 근무하는 거래처 직원 명단이
      ...
      어느 날... 『서울여의도』에 위치한 LG본사가 『경기분당』으로
      사옥을 이전하게 되었다.
      회사 주소는 『경기분당』으로 바뀌고
...
----------------------------------------------
  ...
  20       SK         서울소곡동 02-987-6543  
  30       LG         부산동래구 051-345-6789  
----------------------------------------------
...
-----------------------------------------------------
  ...
  오이삭       대리 oys@te...  010-9900-9900    30
  정윤정       대리 jej@na...  010-2450-8888    20
                                    :
-------------------------------------------------------               
*/
...
--   컬럼 1개로마 구성된 PRIMARY KEY를 SINGLE PRIMARY KEY 라고 부른다.
--   (단일 프라이머리 키)
...
---------------------------------------------------------------------
 ...
 O1150     오라클심화  10      장영실    KT502      전자공학관 5층 60석 규모
...
-- 제 3 정규화
--> 식별자가 안니 컬럼이 식별자가 안니 컬럼에 의존적인 상황이라면
...
------------------------------------- ------------------------------------------
                    ...
                    1002     rhrnak     2022-03-..  20
                                    :
                    -------------------------------------
                    (주문접수 테이블이 다 대 다 관계를 깨뜨림)

*/

-- 제 4 정규화
--> 위에서 확인한 내용과 같이 『many(다) : many(다)』 관계를
--  『1(일) : many(다)』 관계로 깨뜨리는 과정이 바로 제 4 정규화 수행 과정
--  → 파생 테이블 생성 → 다:다 관계를 1:다 관계로 깨뜨리는 역할 수행


-- 역정규화(비정규화) (정규화는 분리였다면 역정규화는 합치는 거다.)
...
-----------------------     ------------------------------------------- + ------
부서번호   부서명  주소      사원번호 사원명 직급 급여 입사일 부서번호    부서명(부서명때문에 부서 테이블을 불러야 하는데 그렇게 안하고 JOIN하는거임)
-----------------------     ------------------------------------------- + ------
        10개 행                           1,000,000 개 행
-----------------------     ------------------------------------------- + ------
...
--> 『사원』 테이블을 역정규화 한 후 이 테이블만 읽어올 때의 크기 (오히려 메모리를 더 쓴다)
--  1000000*70Byte
...
--> 『사원』 테이블을 역정규화 한 후 이 테이블만 읽어올 때의 크기 (역정규화 한게 더 적은 메모리) 
--  1000000*70Byte

-- (달라진게 몇개의 레코드가 들어왔는지만 다른데 유리한게 바뀜 그렇기 때문에 향후에 데이터가 어떤 형태로 커질지에 대한 파악이 필요)
...
3. 튜플==tuple==엔티티==entity
    ...
    - 튜플 변수(tuble variable)
      : 튜플(tuble)을 가리키는 변수, 모든 튜플 집합을 도메인으로 하는 변수
...
-- 2. 테이블에서 각 행을 유일하게 식별하는 컬럼 또는 컬럼의 집합인다.
--    기본 키는 테이블 당 최대 하나만 존재한다.
--    그러나 반드시 하나의 컬럼만으로만 구성되는 것은 아니다.
--    NULL 일 수도 없고, 이미 테이블에 존지하고 있는 데이터를
--    다시 입력할 수 없도록 처리한다.
--    UNIQUE INDEX 가 자동으로 생성된다.(오라클이 자체적으로 만든다.)

3. 형식 및 구조
--  ① 컬럼 레벨의 형식
--     컬럼명 데이터타입 [CONSTAINT CONSTRAIN 명] PRIMARY KEY[(컬럼명, ...)]
...
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(1, 'TEST');    --> 에러 발생(이미 입력된 데이터니까)
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(1, 'ABCD');    --> 에러 발생
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(2, 'ABCD');
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(3, NULL);      
INSERT INTO TBL_TEST1(COL1) VALUES(4);                  --(COL2는 NULL)
INSERT INTO TBL_TEST1(COL1) VALUES(4);                  --> 에러 발생
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(5, 'ABCD');    --(두번 째 컬럼은 내용이 중복되어도 상관없다)
...
--※ USER_CONSTRAINTS 와 USER_CONS_COLUMNS를 대상으로
...
( COL1  NUMBER(5)
, COL2  VARCHAR2(30)
, CONSTRAINT TEST2_COL1_PK PRIMARY KEY(COL1)        --이거 키에 프라이머리 키 지정하는거이고 TEST2_COL1_PK는 이름일뿐
);
...
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(1, 'ABCD');  --> 에러 발생(COL1이 같으니까)
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(2, 'ABCD');
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(3, NULL);
INSERT INTO TBL_TEST2(COL1) VALUES(4);
INSERT INTO TBL_TEST2(COL1) VALUES(4);                --> 에러 발생
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(5, 'ABCD');
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(NULL, NULL); --> 에러 발생 (첫번재 컬럼 때문에)
...
--(이렇게 하면 우리가 지정한 이름인 TEST2_COL1 PK 이렇게 나와서 바로 알 수 있음)
...
INSERT INTO TBL_TEST3(COL1, COL2) VALUES(1, 'ABCD');    --(복합 프라이머리키 들어가면 둘다 같아야 못들어간다는 의미임 그래서 들어감) --주의!
INSERT INTO TBL_TEST3(COL1, COL2) VALUES(2, 'ABCD');
INSERT INTO TBL_TEST3(COL1, COL2) VALUES(3, NULL);      --> 에러 발생(들어가는건 같으면 안되지만 NULL은 따로 따로임)
INSERT INTO TBL_TEST3(COL1) VALUES();                   --> 에러 발생
...
--※ 제약조건 확인 전용 뷰(VIEW) 생성 (매번 저렇게 식 쓰기 귀찮으니까)
