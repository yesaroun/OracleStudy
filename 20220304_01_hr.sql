...
-- 1. 컬럼에서 허용 가능한 범위나 조건을 지정하기 위한 제약조건
...
( COL1 NUMBER(5)    PRIMARY KEY
, COL2 VARCHAR2(30) 
, COL3 NUMBER(3)    CHECK (COL3 BETWEEN 0 AND 100) --(원래 NUMBER(3)는 -999~999까지인데 0에서부터 100까지로 제약조건 설정했음)
);
--==>> Table TBL_TEST8이(가) 생성되었습니다.

-- 데이터 입력
INSERT INTO TBL_TEST8(COL1, COL2, COL3) VALUES(1, '소민', 100);
INSERT INTO TBL_TEST8(COL1, COL2, COL3) VALUES(2, '현수', 101);    --> 에러 발생(100을 넘어서)
...
--==>>
/*
HR	SYS_C007057	TBL_TEST8	C	COL3	COL3 BETWEEN 0 AND 100	--(SEARCH_CONDITION에 우리가 지정한 제약조건이생김)
...
--○ 실습 문제
-- 다음과 같이 TBL_TESTMEMBER 테이블을 새성하여
-- SSN 컬럼(주민번호 컬럼)에서
...
--    다른 테이블에 추가하려면 테이블 간 연결을 설정할 수 있다.
...
--                   [ON DELETE CASCADE | ON DELETE SET NULL]       → 추가 옵션 (부모테이블을 지우면 하위 자식 테이블도 같이 지워지는 옵션, 가급적쓰면 안되는 옵션(위험함))
...
--   그리고 이때, 부모 테이블에는 반드시 PK 또는 UK제약조건이
--   설정된 컬럼이 존재해야 한다.
...
HR	SYS_C007067	TBL_EMP1	R	JIKWI_ID		NO ACTION  --(NO ACTION: 그 앞에서 말한 옵션이 부여되지 않음을 말함)
*/
...
INSERT INTO TBL_EMP1(SID, NAME, JIKWI_ID) VALUES(6, '오이삭', NULL); --(부모테이블 데이터 선택하든지 아니면 아예 비워두면 된다)
INSERT INTO TBL_EMP1(SID, NAME) VALUES(7, '박현지'); -- (위랑 같은 구문)
...
--    PRIMARY KEY 제약조건이 설정된 컬럼의 데이터 삭제시
--    FOREGIN KEY 컬럼에 그 값이 입력되어 있는 경우 삭제되지 않는다.
...
-- (부모테이블이 갖고 있는 데이터도 삭제가 안되고 부모테이블을 지우는 것도 불가 왜냐하면 자식 테이블이 에 연결된게 있어서)
...
-- 컬럼명 데이터 타입,
-- 컬럼명 데이터 타입,
-- CONSTRAINT CONSTRAIN명 CHECK(컬럼명 IS NOT NULL)
...
COL1 NOT NULL NUMBER(5)    
COL2          VARCHAR2(30) 
*/ -- (그래서 이것만 가지고 COL2에 NOT NULL 제약조건이 걸려있는지 확인하기 어렵다)
...
-- 3. INSDRT 명령 시 해당 컬럼에 입력될 값을 할당하지 않거나,
...
VALUES(SEQ_BBS.NEXTVAL, '이연주', '열심히 실습중입니다.'
     , DEFAULT, 0, 0);              --(앞에서 DEFAULT를 뇬ㅇㅁㅅ드로 해서 가능)
--==>> 1 행 이(가) 삽입되었습니다.
...
VALUES(SEQ_BBS.NEXTVAL, '김태형', '무진장 실습중입니다.');      --(원래는 이렇게 갑 입력안하면 NULL이지만 DEFAULT 표현식쓰면 NULL이 아니고 DEFAULT한 값임)
--==>> 1 행 이(가) 삽입되었습니다.
...
2	이호석	계속 실습중입니다.	2022-03-04 14:29:57	0	0
3	이연주	열심히 실습중입니다.	2022-03-04 14:30:43	0	0
4	김태형	열심히 실습중입니다.	2022-03-04 14:31:55	0	0
